...
- Built-manager non-dry launch: CORRECT for process start, argument identity, collection-selector bypass, save-payload preservation, and window close. The earlier dry-run of `dist\mgs3mod.exe` exited 0 and wrote no preference file. On 2026-09-21 the installed `mgs3mod.exe launch --profile na-startup --json` exited 0 for PID 52380 with the supported argument array. No launcher process was present. A capture showed the in-game DATA LOAD screen. `CloseMainWindow` exited that PID. Game-save hashes were unchanged, `verify --json` stayed at generation 15, and `mgs3mod-launch.json` remained absent. Details are Trial 4 in [direct-launch-experiments.md](direct-launch-experiments.md). Crouch Walk was not played during this launch, and the frame before DATA LOAD was not captured.
