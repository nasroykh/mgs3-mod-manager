...
Hosted Windows runners may use an 8.3 alias in TEMP and grant parent-directory DELETE_CHILD rights. The workflow uses a canonical scratch path under `work/test-temp` and denies that alternate delete-child route on its test directories. File DELETE rights remain available until a fixture explicitly denies them. This keeps strict production path validation and the negative ACL test meaningful without skipping tests or changing game-directory permissions.
